...
class DeviceData:
    ...
    def __parse_nfs_line(self, words):
        if words[0] == 'device':
            ...
            if self.__nfs_data['age'] == 0:
                self.__nfs_data['age'] = 1 ## prevents div/0 errors on very new mounts
        elif words[0] == 'opts:':
    ...
    def get_rpc_op_stats(self, op, sample_time):
        """Calculate generic stats for one RPC op
        """
    ...
    def __print_rpc_op_stats(self, op, sample_time):
        ...
        """
        stats = self.get_rpc_op_stats(op, sample_time)
        if not stats:
            return
    ...
    def display_iostats(self, sample_time, which):
        ...
        print '%7.2f' % self.ops(sample_time)
        print '\t%7.2f' % self.backlog(sample_time)
